...
resource "aws_db_instance" "fastapi_app" {
...
resource "aws_secretsmanager_secret_version" "db_connection_string" {
  ...
  secret_string = "postgresql://${aws_db_instance.fastapi_app.username}:${random_password.db_password.result}@${aws_db_instance.fastapi_app.endpoint}/${aws_db_instance.fastapi_app.name}"
}